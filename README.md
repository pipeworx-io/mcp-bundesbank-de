...
```bash
curl -X POST https://gateway.pipeworx.io/v1/tools/bundesbank_de_list_dataflows \
  -H 'Content-Type: application/json' \
  -d '{}'
```

No account needed for the first calls. Inspect any tool: `GET https://gateway.pipeworx.io/v1/tools/bundesbank_de_list_dataflows`. Find one: `POST https://gateway.pipeworx.io/v1/tools/search_packs` with `{"query":"..."}`.
...
```json
{
  "mcpServers": {
    "bundesbank-de": {
      "command": "npx",
      "args": ["-y", "@pipeworx/mcp-bundesbank-de"]
    }
  }
}
```
...
```bash
npx -y @pipeworx/mcp-bundesbank-de
```
